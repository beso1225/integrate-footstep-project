...
class IntegrationFilesTest(unittest.TestCase):
    ...
    def test_tracker_enables_indoor_emotion_by_default(self):
        ...
        config_module = (PYTHON_DIR / "tracker_config.py").read_text()
        emotion_module = (PYTHON_DIR / "tracker_emotion.py").read_text()
        runtime_module = (PYTHON_DIR / "tracker_runtime.py").read_text()

        self.assertIn("from tracker_runtime import run", tracker)
        ...
        self.assertTrue((PYTHON_DIR / "tracker_config.py").is_file())
        self.assertTrue((PYTHON_DIR / "tracker_emotion.py").is_file())
        self.assertTrue((PYTHON_DIR / "tracker_runtime.py").is_file())
